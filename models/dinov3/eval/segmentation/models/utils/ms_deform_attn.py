...
class MSDeformAttnFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        dtype_sampling_loc = sampling_locations.dtype
        dtype_attn_weight = attention_weights.dtype

        temp_dtype = sampling_locations.dtype
        ...
        grad_value, grad_sampling_loc, grad_attn_weight = MSDA.ms_deform_attn_backward(
            ...
            value_level_start_index,
            sampling_locations,
            attention_weights.to(temp_dtype),
            grad_output.to(temp_dtype),
            ctx.im2col_step,
        )

        grad_value = grad_value.to(dtype_value)
        grad_sampling_loc = grad_sampling_loc.to(dtype_sampling_loc)
        grad_attn_weight = grad_attn_weight.to(dtype_attn_weight)
...
def ms_deform_attn_core_pytorch(value, value_spatial_shapes, sampling_locations, attention_weights):
    ...
    for lid_, (H_, W_) in enumerate(value_spatial_shapes):
        ...
        # N_*M_, D_, Lq_, P_
        orig_dtype = value_l_.dtype
        temp_dtype = sampling_grid_l_.dtype
        sampling_value_l_ = F.grid_sample(
            value_l_.to(temp_dtype), sampling_grid_l_, mode="bilinear", padding_mode="zeros", align_corners=False
        )
        sampling_value_l_ = sampling_value_l_.to(orig_dtype)
