...
from models.dinov3.layers import SelfAttentionBlock, SwiGLUFFN
from models.dinov3.models.vision_transformer import init_weights_vit
...
class VisionHead(nn.Module):
    ...
    def forward(self, image_tokens: torch.Tensor, **kwargs) -> torch.Tensor:
        # FIXME(cijose) ROPE embeddings are not used in DINOv2, refactor to use it in the future
        attn_mask = kwargs.get("attn_mask", None)
        for block in self.blocks:
            image_tokens = block(image_tokens, attn_mask_or_mask_list=attn_mask)
...
def build_vision_model(
    ...
    if backbone is None:
        if backbone_model_config is not None:
            ...
            from models.dinov3.models import build_model_from_cfg as build_vision_backbone
