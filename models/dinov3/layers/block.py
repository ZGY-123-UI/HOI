...
class SelfAttentionBlock(nn.Module):
    ...
    def _forward(self, x: Tensor, rope=None, attn_mask=None) -> Tensor:
        """
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            # Index the attention mask if provided
            attn_mask_subset = None
            if attn_mask is not None:
                attn_mask_subset = attn_mask[indices_1]
            
            residual_1 = self.attn(self.norm1(x_subset_1), rope=rope_subset, attn_mask=attn_mask_subset)
    ...
    def _forward_list(self, x_list: List[Tensor], rope_list=None, attn_mask_list=None) -> List[Tensor]:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            attn_mask_subset_list = None
            if attn_mask_list is not None:
                attn_mask_subset_list = [
                    attn_mask[indices_1] if attn_mask is not None else None
                    for attn_mask, indices_1 in zip(attn_mask_list, indices_1_list)
                ]
            ...
            residual_1_list = self.attn.forward_list(
                ...
                attn_mask_list=attn_mask_subset_list, 
            )
            ...
            for x, rope, attn_mask in zip(x_list, rope_list, attn_mask_list):
                x_attn = x + self.ls1(self.attn(self.norm1(x), rope=rope, attn_mask=attn_mask))
                x_ffn = x_attn + self.ls2(self.mlp(self.norm2(x_attn)))
                x_out.append(x_ffn)
            x_ffn = x_out
    ...
    def forward(self, x_or_x_list, rope_or_rope_list=None, attn_mask_or_mask_list=None) -> Tensor | List[Tensor]:
        if isinstance(x_or_x_list, Tensor):
            ...
            return self._forward_list(
                ...
                attn_mask_list=[attn_mask_or_mask_list] if attn_mask_or_mask_list is not None else None,
            )[0]
        elif isinstance(x_or_x_list, list):
            if rope_or_rope_list is None:
                rope_or_rope_list = [None for x in x_or_x_list]
            if attn_mask_or_mask_list is None:
                attn_mask_or_mask_list = [None for x in x_or_x_list]
            return self._forward_list(
                x_or_x_list, 
                rope_list=rope_or_rope_list,
                attn_mask_list=attn_mask_or_mask_list,
            )
        else:
